import type {NextResponse,  NextRequest} from 'next/server';
import { getDbInstance } from '../../../../../lib/database-init';
import type { ParticipantDbRow, MatchDbRow } from '@/shared/types';
import { logger } from '@/lib/logger';
import { apiError, apiOk } from '@/lib/api-response';

export async function GET(
  request: NextRequest,
  { params }: { params: Promise<{ matchId: string }> }
): Promise<NextResponse> {
  try {
    const db = await getDbInstance();
    const { matchId } = await params;
    
    // Check if match exists and get game info
    const match = await db.get<MatchDbRow>(
      'SELECT m.id, m.game_id, g.id as game_name FROM matches m JOIN games g ON m.game_id = g.id WHERE m.id = ?',
      [matchId]
    );
    
    if (!match) {
      return apiError('Match not found', 404);
    }
    
    // Fetch all participants for this match
    const participants = await db.all<ParticipantDbRow & { team_id?: string | null }>(`
      SELECT id, user_id, discord_user_id, username, avatar_url, joined_at, signup_data, team_assignment, team_id, receives_map_codes
      FROM match_participants
      WHERE match_id = ?
      ORDER BY joined_at ASC
    `, [matchId]);
    
    // Parse signup_data JSON for each participant if it exists
    const parsedParticipants = participants.map(participant => ({
      ...participant,
      signup_data: participant.signup_data ? JSON.parse(participant.signup_data) : null
    }));
    
    // Fetch signup form configuration to get field labels
    let signupConfig = null;
    try {
      // Validate game_id to prevent path traversal
      if (/^[a-z0-9_-]+$/i.test(match.game_id)) {
        const { readFile, access } = await import('fs/promises');
        const path = await import('path');
        const signupPath = path.join(process.cwd(), 'data', 'games', match.game_id, 'signup.json');

        const exists = await access(signupPath).then(() => true).catch(() => false);
        if (exists) {
          const raw = await readFile(signupPath, 'utf8');
          signupConfig = JSON.parse(raw);
        }
      }
    } catch {
      logger.debug('No signup form config found for game:', match.game_id);
    }
    
    return apiOk({
      participants: parsedParticipants,
      signupConfig: signupConfig
    });
  } catch (error) {
    logger.error('Error fetching match participants:', error);
    return apiError('Failed to fetch participants');
  }
}