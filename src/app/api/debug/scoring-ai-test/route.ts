...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!gameId) {
      return apiError('Game ID is required', 400);
    }
    if (!imageFile) {
    ...
    if (!validTypes.includes(imageFile.type)) {
      return apiError('Invalid file type. Must be jpeg, png, webp, or gif.', 400);
    }
    ...
    if (!statDefs || statDefs.length === 0) {
      return apiError(`No stat definitions found for game ${gameId}`, 400);
    }

    const game = await db.get<{ name: string }>('SELECT name FROM games WHERE id = ?', [gameId]);
    if (!game) {
      return apiError(`Game ${gameId} not found`, 404);
    }
    ...
    if (enabledProviders.length === 0) {
      return apiError('No providers enabled. Configure them in Stats Settings.', 400);
    }
    ...
    return apiError('Failed to run AI test');
  }
}
